...
However, **7 bugs were discovered** during testing, including 2 critical issues that can cause cascading failures.
...
- **Status**: Open - needs upsert logic
...
- **Status**: Open - needs pagination/caching
...
1. Apply storage RLS migration (`supabase/migrations/20260119_fix_audio_storage_rls.sql`)
2. Deploy onboarding completion fix

### High Priority
3. Add upsert logic for sentence generation (#26)
4. Optimize progress API queries (#28)

### Medium Priority
5. Fix nested button accessibility (#27)

### Low Priority
6. Add turbopack config (#29)
7. Improve sentence validation (#30)
