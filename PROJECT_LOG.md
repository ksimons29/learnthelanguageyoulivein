...
### Recently Completed
- [x] **Inbox Category Picker + Playwright E2E Suite (#167, #140) (Session 113)** - Implemented two features: (1) **#167 Inbox organization**: Added `updateWordCategory()` API action to words-store and interactive category picker to WordDetailSheet - users can now change word categories directly from the detail sheet with a 2-column grid showing all 8 categories. (2) **#140 Automated E2E tests**: Created Playwright test suite with 4 test files (auth.spec.ts, capture.spec.ts, review.spec.ts, notebook.spec.ts), test fixtures for all 3 language directions, CI workflow (.github/workflows/e2e.yml), and npm scripts (test:e2e, test:e2e:ui). **Documentation update**: CLAUDE.md and web/CLAUDE.md now enforce automatic E2E test execution - Claude Code MUST run `npm run test:e2e` after changes to capture, review, notebook, today, auth, stores, fsrs, or sentences. No asking, no skipping. TESTING.md updated with comprehensive E2E test section. Build passes, 459 tests pass.
...
| `web/e2e/` | Playwright E2E test suite (auth, capture, review, notebook) |
| `web/playwright.config.ts` | Playwright configuration (baseURL, projects, timeouts) |
| `.github/workflows/e2e.yml` | CI workflow for automated E2E tests on PR/push |
